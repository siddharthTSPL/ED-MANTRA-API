...
router.post('/api/addMarketingLeads', authenticate(accecableModules), async (req, res) => {
    try {
        // Extract lead data from the request body
        const leadData = req.body;

        // Call singleInsertData function to insert the lead data
        const success = await MarketingLeads.create(leadData); // Pass leadData directly

        if (success) {
            // If insertion is successful, fetch and return sorted leads
            const sortedLeads = await MarketingLeads.findAll({ order: [['SrNo', 'ASC']] });
        ...
        if (error.name === "SequelizeUniqueConstraintError" && error.fields && error.fields.pocMobile) {
